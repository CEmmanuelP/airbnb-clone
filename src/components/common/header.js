        ...
        <div className="div-header-menus">
          ...
          <div className="user">
            <Link to="#" onClick={onClickUserButton}>
              <FontAwesomeIcon icon={faBars} className="text-2xl menu" />
              <FontAwesomeIcon icon={faUser} className="text-2xl userIcon" />
            </Link>
            {userMenuModal &&
              (users ? <LoginUserMenuModal /> : <UserMenuModal />)}
          </div>
        </div>
        {/* search menu */}
        <form className="search" action="/houses">
          {scrolled ? (
            <div className="scrolled-placeholder">검색 시작하기</div>
          ) : (
            ""
          )}
          <div className="overlay">
            <div className="field">
              ...
              <input id="location" placeholder="어디로 여행가세요?" />
            </div>
